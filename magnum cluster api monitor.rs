    ...
    fn poll_health_status(&self, py: Python<'_>) -> PyResult<PyObject> {
        ...
        let result = py.allow_threads(move || {
            thread::spawn(move || {
                ...
                // Run the asynchronous code within the Tokio runtime
                rt.block_on(async {
                    let stack_id = match &cluster.stack_id {
                        Some(id) => id,
                        None => {
                            return Err("Cluster has no stack ID".to_string());
                        }
                    };
                    ...
                    let list_params = ListParams::default().labels_from(
                        ...
                        .try_into()
                        .map_err(|e| format!("Failed to parse label selector: {}", e))?,
                    );

                    let machine_api: Api<Machine> = Api::namespaced(client.clone(), "magnum-system");
                    let kcp_api: Api<KubeadmControlPlane> =
                        Api::namespaced(client.clone(), "magnum-system");

                    let (machines_result, kcp_list_result) =
                        futures::join!(machine_api.list(&list_params), kcp_api.list(&list_params));

                    let machines = machines_result
                        .map_err(|e| format!("Failed to get machines: {}", e))?;
                    let kcp_list = kcp_list_result
                        .map_err(|e| format!("Failed to get KubeadmControlPlane: {}", e))?;

                    let kcp = kcp_list
                        .items
                        .into_iter()
                        .next()
                        .ok_or_else(|| format!("No KubeadmControlPlane found for cluster: {}", stack_id))?;

                    let is_healthy =
                        kcp.is_ready() && machines.items.iter().all(|machine| machine.is_ready());

                    let health_status_reason_obj = machines.items.to_health_status_reason();
                    let reason_map = health_status_reason_obj.0;
                    let api_status = if kcp.is_ready() { "ok" } else { "nok" };

                    Ok((is_healthy, reason_map, api_status))
                })
            })
            .join()
            .unwrap()
        ...
        let health_status_reason_py = PyDict::new(py);

        match result {
            Ok((is_healthy, reason_map, api_status)) => {
                data.set_item("health_status", if is_healthy { "HEALTHY" } else { "UNHEALTHY" })?;
                for (name, ready) in reason_map {
                    health_status_reason_py.set_item(name, if ready { "True" } else { "False" })?;
                }
                health_status_reason_py.set_item("api", api_status)?;
                data.set_item("health_status_reason", health_status_reason_py)?;
                Ok(data.into())
            }
            Err(e) => Err(PyException::new_err(format!("Error polling health status: {}", e))),
        }
    }
}
